 ...
 *------------------------------------------------------------------------------ */
/* ./iirGen.pl -A 256 256 256 256 256 0 0 0 0 0 0 0 -B 0 4 22 68 136 191 191 136 68 22 4 0 -moduleName acm_filter -fractionalBits 8 -bitWidth 32 -inData inData -inReg -outReg -outData outData -clk clk -reset reset -reset_edge negedge -filterForm 1 -outFile ../outputs/filter_1527974994.v */
...
module acm_filter_firBlock_left_MultiplyBlock (
  ...
  assign w4      = w1  << 2;  //X*4
  assign w22     = w11 << 1;  //X*22
  assign w68     = w17 << 2;  //X*68
  assign w136    = w17 << 3;  //X*136
  assign w191    = w192 - w1; //X*191
  //assign w191  = w192 - w1; //X*191
  //assign w136  = w17 << 3;  //X*136
  //assign w68   = w17 << 2;  //X*68
  //assign w22   = w11 << 1;  //X*22
  ...
  assign Y[0] =  X*coef[7:0];//w4[39:8];
  assign Y[1] =  X*coef[15:8];//w22[39:8];
  assign Y[2] =  X*coef[23:16];//w68[39:8];
  assign Y[3] =  X*coef[31:24];//w136[39:8];
  assign Y[4] =  X*coef[39:32];//w191[39:8];
  assign Y[5] =  X*coef[47:40];//w191[39:8];
  assign Y[6] =  X*coef[55:48];//w136[39:8];
  assign Y[7] =  X*coef[63:56];//w68[39:8];
  assign Y[8] =  X*coef[71:64];//w22[39:8];
  assign Y[9] =  X*coef[79:72];//w4[39:8];
  assign Y[10] =   w0[39:8];

  //acm_filter_firBlock_left_MultiplyBlock area estimate = 9360.48955432647;
...
module acm_filter_firBlock_left (
  ...
  //acm_filter_firBlock_left area estimate = 56049.8412354117;
...
module acm_filter_firBlock_right_MultiplyBlock (
  ...
  assign Y[0] = w256_[39:8];
  assign Y[1] = w256_[39:8];
  assign Y[2] = w256_[39:8];
  assign Y[3] = w256_[39:8];

  //acm_filter_firBlock_right_MultiplyBlock area estimate = 1437.00483871323;
...
module acm_filter_firBlock_right (
  ...
  assign Y_in = firStep[2]+ multProducts[3];
  //acm_filter_firBlock_right area estimate = 17816.1988887691;
...
module FIR_filter (
  ...
  //acm_filter area estimate = 80957.9251202543;
